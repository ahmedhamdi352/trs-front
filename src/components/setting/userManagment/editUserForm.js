...
const UserForm = ({ roles, userId, defaultValues, model }) => {
  const { loading } = useSelector(({ setting }) => setting.updateUser);

  const dispatch = useDispatch();
  const { handleSubmit, control, formState, errors, reset } = useForm({
    defaultValues: { firstName: '', lastName: '', username: '', email: '', phone: '', isActive: true, role: '' },
    mode: 'onChange',
  ...
  const onSubmit = (values) => {
    const role = roles.filter(item => item.name === defaultValues.role)
    if (!isEmpty(role)) {
      dispatch(updateUser(userId, { ...values, role: role[0]?.internalId }, model));
    }
  };
  return (
